...
let lb={photos:[],i:0};
let modalState={photos:[],i:0,id:null};
...
function openModal(id){var p=PRODUCTS.find(function(x){return x.id===id;});if(!p)return;var t=T();modalState={photos:p.photos,i:0,id:id};var pt=priceText(p);var thumbs=p.photos.map(function(src,i){return '<div class="mthumb'+(i===0?' on':'')+'" data-i="'+i+'"><img src="'+src+'" loading="lazy" alt=""/></div>';}).join('');var buy='';if(p.sold){buy='<span class="soldbtn">'+t.sold+'</span>';}else{if(CONTACT.telegram){buy+='<button class="btn tg buy" data-id="'+p.id+'">'+t.tg+'</button>';}buy+='<button class="btn em lnk" data-id="'+p.id+'">'+t.copylink2+'</button>';}var info='<div class="mnm">'+p.name+'</div><div class="mln">#'+p.id+' · '+p.line+' · '+p.year+' · '+epLabel(p.ep)+'</div><div class="mcond">'+condText(p)+'</div><div class="mpr">'+pt.big+(pt.obo?' <span class="obo">'+pt.obo+'</span>':'')+'</div><div class="mbtns">'+buy+'</div>';document.getElementById('mbox').innerHTML='<span class="mclose" onclick="closeModal()">&times;</span><div class="mthumbs">'+thumbs+'</div><div class="mmainwrap"><div class="mmain"><img id="mmainimg" src="'+p.photos[0]+'" alt=""/></div><div class="mzoomhint">'+t.zoomhint+'</div></div><div class="minfo">'+info+'</div>';document.getElementById('modal').classList.add('on');}
function setMain(i){modalState.i=i;document.getElementById('mmainimg').src=modalState.photos[i];var ths=document.querySelectorAll('#mbox .mthumb');for(var k=0;k<ths.length;k++){ths[k].classList.toggle('on',(+ths[k].getAttribute('data-i'))===i);}}
...
document.getElementById("grid").addEventListener("click",function(e){var ph=e.target.closest(".ph");if(ph){openModal(ph.getAttribute("data-id"));return;}var b=e.target.closest(".buy");if(b){contactTG(b.getAttribute("data-id"));return;}var l=e.target.closest(".lnk");if(l){copyItemLink(l.getAttribute("data-id"));return;}});
document.getElementById("modal").addEventListener("click",function(e){if(e.target.id==="modal"){closeModal();return;}var th=e.target.closest(".mthumb");if(th){setMain(+th.getAttribute("data-i"));return;}if(e.target.id==="mmainimg"){lb.photos=modalState.photos;lb.i=modalState.i;document.getElementById("lbimg").src=lb.photos[lb.i];document.getElementById("lb").classList.add("on");return;}var b=e.target.closest(".buy");if(b){contactTG(b.getAttribute("data-id"));return;}var l=e.target.closest(".lnk");if(l){copyItemLink(l.getAttribute("data-id"));return;}});
